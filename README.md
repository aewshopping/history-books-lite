...
`header` is set to `X-Forwarded-For` — confirmed correct for **Vercel**, the intended production target: Vercel's edge network populates this header with the real client IP ([docs](https://vercel.com/docs/headers/request-headers)). Verified locally (with a header actually present on requests) that it correctly triggers 429s per-IP, and doesn't false-block a different IP sharing the same window.

Two caveats worth knowing before trusting this in production:

- **Codespaces testing won't reflect this correctly.** Earlier testing here found that when `header` is set but the named header isn't actually present on a request (as in a plain Codespaces terminal `curl`, or possibly Codespaces' own port-forwarding), the plugin **silently stops rate-limiting entirely** rather than falling back to the connection address — worse than doing nothing. So local testing in Codespaces may show *no* rate limiting even though it's configured, unless you explicitly send an `X-Forwarded-For` header yourself when testing (or verify with the plugin's `debug: true` option, which exposes `/-/ip-rate-limit-debug`).
- **On Vercel specifically, the header being correct doesn't make this a hard guarantee.** The plugin tracks per-IP counts in Python process memory, with no persistent/shared store (e.g. no Redis). Vercel's Python functions have no persistent server process — under real load, requests can be spread across multiple concurrently-scaled instances (each with separate memory) or hit a cold-started instance with a reset counter. So this is **best-effort noise reduction**, not a reliable defense against a determined or bursty scraper; the `allow_sql` token gate is the actual hard boundary for the sensitive part (arbitrary SQL execution).

Adjust `window_seconds`/`max_requests`/`block_seconds` in `metadata.json` if these defaults are too strict or too loose for real usage.
